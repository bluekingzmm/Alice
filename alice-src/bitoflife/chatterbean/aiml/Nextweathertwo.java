...
public class Nextweathertwo extends TemplateElement {
	/*
	 * 明天的天气。匹配第二个*
	 */
	...
	public Nextweathertwo(int index) {
		this.index = index;
	}

	public Nextweathertwo() {

	}

	public Nextweathertwo(Attributes attributes) {
		String value = attributes.getValue(1);
		index = (value != null ? Integer.parseInt(value) : 2);
	}
	...
	public String toString() {
		return "明天天气：" + date + " [城市:" + city + ",最低温度:" + minTmp+",最高温度:"+maxTmp + ",湿度:" + hum + ",天气:" + cond1+"转"+cond2+",风向:"+wind+"]";
	}


	public String process(Match match) {
		Nextweathertwo wea = null;
		String url = "";
		try {
			...
			for (int i = 0; i < array.length(); i++) {
				wea = new Nextweathertwo();
				JSONArray a = array.getJSONObject(i).getJSONArray("daily_forecast");
				for (int j = 0; j < a.length(); j++) {
					wea.setDate(a.getJSONObject(1).get("date").toString());
					wea.setHum(a.getJSONObject(1).get("hum").toString());
					wea.setMinTmp(a.getJSONObject(1).getJSONObject("tmp").getString("min"));
					wea.setMaxTmp(a.getJSONObject(1).getJSONObject("tmp").getString("max"));
					wea.setCond1(a.getJSONObject(1).getJSONObject("cond").get("txt_n").toString());
					wea.setCond2(a.getJSONObject(1).getJSONObject("cond").get("txt_d").toString());
					wea.setWind(a.getJSONObject(1).getJSONObject("wind").get("dir").toString());
				}
		...
		return (wea.getCity() != null ? wea.toString().trim() : "不太懂哦！");
	}
